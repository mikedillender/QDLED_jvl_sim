...
dd2 = 1.5e-7
# Define the mesh
x = np.concatenate((np.linspace(0, dd, 80, endpoint=False),                        # L contact interface
                    np.linspace(dd, t_hil-dd2, 50, endpoint=False),                    # material 1
                    np.linspace(t_hil - dd2, t_hil + dd2, 50, endpoint=False),             # interface 1
                    np.linspace(t_hil + dd2, (t_bqd) - dd, 50, endpoint=False),       # material 2
                    np.linspace((t_bqd) - dd, (t_bqd), 30, endpoint=False),      # htl-qd interface
                    [(t_bqd)+r_qd, (t_aqd)-r_qd],      # QD
                    np.linspace((t_aqd), (t_aqd) + dd, 30, endpoint=False),      # qd-etl interface
                    np.linspace((t_aqd) + dd, (t_total) - dd, 100, endpoint=False),       # material 2
                    np.linspace((t_total) - dd, (t_total), 80)))                       # R contact interface
# Build system
...
Lcontact_workFcn, Rcontact_workFcn = 5.4, 4.15   # Lcontact work function irrelevant because L contact is Ohmic
# Add the contacts
